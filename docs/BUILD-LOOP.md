...
| Audit 2026-07-07 wave-1 fixes (loops/sync/downloads/UX/perf) | done | Full-codebase audit (`docs/AUDIT-2026-07-07.md`, 5 parallel deep-read audits over `b9ff46f`; findings keyed G/S/D/U/F/T) then a wave-1 fix pass, each fix with a named test. G1: `spec_block_equals_defaults` made platform-aware (the CLIP default went fp16-on-CUDA / int8-on-macOS-CPU but the test compared the spec literal verbatim — gate was red on clean main beyond s02_2). S1/S2: startup `reconcile_all` off the doctor thread + a pump wall-clock-gap resume detector (`on_system_resume` was dead code) — the grid no longer shows ghost/stale assets for ~10 min after launch or after sleep. S3/S4: one post-commit `bump_images_version` chokepoint covers supersede/relink/observe_removed/rename-relink/went-stale (in-place edits, live moves, live deletes now re-list the grid; idle reconciles pinned NOT to churn). U1: Diversify no longer silently hides newly-ingested photos (fold keyed on the report `hidden` set, shared `refreshLensesIfScopeChanged`); U2/U4/U5/U6/U7 lens fixes. F1: `photoproof://` unbounded thread-per-request replaced with a fixed core-count pool (T1 `preview_serve_latency`: 200 concurrent serves 2.78 ms, warm p99 0.089 ms on M1). F2: O(1) Set-backed selection membership (10k Select-All scroll 1363 ms -> 0.72 ms); F3: micro (96 px) thumb tier for small cells (~150 MB -> ~5 MB decoded bitmaps zoomed out); T3 vitest perf budgets. D1/D2/D3/D5: download disk preflight, retryable 429/5xx, cancel button, live throughput line. Gate on the final tree: `cargo fmt --all --check` clean; `cargo clippy --workspace --all-targets` 0 warnings; `cargo test --workspace` 805 passed / 30 ignored / 1 failed (s02_2 known); vitest 92 files 1194 passed; svelte-check 0/0 across 459 files; `vite build` ok; check:emdash clean; T1 release perf test green. Deferred items (S5/S6/D4/D6/D7/D8/U3/U8/F4/F5/F6/F7/T2/T4) logged in BACKLOG.md. | `c2617cb`…`0a2d957` |
