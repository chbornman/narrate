...
The five natural context levels (single image → recent images → current folder → multiple folders/projects → whole library) must not behave uniformly. The design splits them:
...
4. **Active projects** — notes and membership from the intent-memory store, independent of folder structure.
5. **Whole library** — never loaded wholesale; reached only through hybrid retrieval.

Context assembly fills a token budget in that order: selection gets full text, recency gets summaries, folder gets its rollup, projects contribute their notes, and the library contributes only what search pulls in.
...
4. **Structured filters** — date ranges, project membership, EXIF, rating events.
...
**M3 — Retrieval.** Embedding pipeline (annotations + images), hybrid search, project memory store, natural-language query.
