...
describe("pointer-reachability audit (dogfood round 1: visible UI for actions)", () => {
  ...
  const CHROME_OR_POINTER_NATIVE: Record<string, string> = {
    ...
    "ui-zoom": "native View menu Zoom In / Zoom Out rows (menu.rs → menu-action event)",
    "ui-zoom-reset": "native View menu Reset Zoom row (menu.rs → menu-action event)",
    ...
    "mic-press":
      'indicator mic segment click (Indicator.svelte via resolveAction, arg "toggle" — a click is a tap; a menu item cannot hold)',
