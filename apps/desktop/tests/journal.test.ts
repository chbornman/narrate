...
describe("source/kind chip (BACKLOG 'Journal entry type/source chips')", () => {
  it("a drawing reads as Pencil from its KIND, not its source", () => {
    // There is no "pencil" source value in the DTO - stroke rows are the
    // drawing, whatever their source enum says. Stroke wins over source.
    const chip = sourceChip(entry({ id: "01A", kind: "stroke", text: null }));
    expect(chip).toEqual({
      variant: "pencil",
      label: "Pencil",
      title: "Pencil - a drawing",
    });
  });

  it("a voice remark reads as Voice", () => {
    expect(sourceChip(entry({ id: "01A", source: "voice" }))).toEqual({
      variant: "voice",
      label: "Voice",
      title: "Voice - a spoken note",
    });
  });

  it("a typed remark reads as Typed", () => {
    expect(sourceChip(entry({ id: "01A", source: "typed" }))).toEqual({
      variant: "typed",
      label: "Typed",
      title: "Typed - a written note",
    });
  });

  it("a system-authored entry reads as the Summary tag (lights up when generation lands)", () => {
    expect(sourceChip(entry({ id: "01A", source: "system" }))).toEqual({
      variant: "summary",
      label: "Summary",
  ...
  it("contains no em-dash in any user-visible chip copy (project rule)", () => {
    for (const src of ["voice", "typed", "system"] as const) {
      const chip = sourceChip(entry({ id: "01A", source: src }));
