/**
 * Topic-note display logic (logic/topic-notes.ts, the collection-notes mirror):
 * chronological (oldest-first) ordering by append order, and the per-note
 * local-time stamp the rail view renders. A topic note is the user's authored
 * text about the topic, append only - the list reads the way it accreted.
 */
import { describe, expect, it } from "vitest";
import { chronological, formatNoteStamp } from "../src/lib/logic/topic-notes";
...
const note = (id: string, text = "what this topic is for"): TopicNoteDto => ({
  id,
  ts: ts(2026, 6, 13, 14, 2),
  text,
});
...
describe("formatNoteStamp (local-time, the diary register)", () => {
  it("renders date and zero-padded 24h time", () => {
    expect(formatNoteStamp(ts(2026, 6, 13, 14, 2))).toBe("13 Jun 2026, 14:02");
  });
