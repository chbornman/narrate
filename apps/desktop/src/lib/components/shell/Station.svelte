<script lang="ts">
   ...
   * gate on the def (resolveAction), never here. */
  function seatClick(actionId: Parameters<typeof resolveAction>[0]) {
    const action = resolveAction(actionId, ui.actionContext());
...
>
  ...
  <div bind:this={capsuleEl} class="capsule" class:breathing={model.pulsing} class:pulsing>
    ...
    {#each model.seats as seat (seat.id)}
      ...
      <button
        class="seat"
        onclick={() => seatClick(seat.actionId)}
        aria-label={seat.title}
        title={seat.title}
        {@attach tooltip({ actionId: seat.actionId })}
      >
        <span
          class="segment glyph"
