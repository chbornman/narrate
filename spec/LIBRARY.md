...
Siblings: EVENTS, SIDECARS, CAPTURE, RETRIEVAL, RUNTIME, UI. Kernel terms (BLAKE3 identity,
append-only log, sidecars-as-truth, merge=union) are assumed, not restated. This document is
the **index side** of the system: every table here is rebuildable from the filesystem plus
the sidecar set. The event log (EVENTS.md) and sidecars (SIDECARS.md) are truth; nothing in
this spec is.
...
Identity = **BLAKE3-256 of the complete file bytes**, 64 lowercase hex characters. This
string is `image_hash` everywhere: DB, sidecars, event targets, cache filenames, manifests.
Hashing is over the file as stored (RAW container, JPEG, …) — no pixel-level or
metadata-stripped hashing in v1. Images are known by hash, never by path; paths are claims
about where a hash's bytes were last observed (§3).
...
A re-export overwriting `IMG_4471.jpg` is **a new image identity**; Photoproof never guesses
whether an overwrite is "the same picture." When bytes at a known path change (detected per
§7):
...
*content* (fan-out across duplicate locations is SIDECARS.md's jurisdiction). The `images`
row carries the EXIF subset (§9.6), pixel geometry, and format classification — all derived,
all rebuildable. No "delete image" operation exists in v1: files vanishing make an image
`missing`; journals and events are never deleted (redaction, per EVENTS.md, operates on
events, not images).
...
A path row asserts: "this image's bytes were observed at `(volume_id, rel_path)` with this
size and mtime."
...
- `rel_path` is relative to the **volume root**, UTF-8, `/`-separated on all platforms.
  ...
  Comparisons are byte-wise; a case-only rename on a case-insensitive filesystem is an
  ordinary rename (relink, §7.4).
- Symlinks are **not followed**. Hard links are naturally fine: same bytes, two paths, one
  image.
...
watched root** beats outside; (3) **writable** volume beats read-only; (4) most recent
`last_verified_at`; (5) lexicographically smallest `(volume_id, rel_path)` as the
...
2. **Platform-native id**: macOS — volume UUID via `DADiskCopyDescription`
   ...
   used, it is unstable across mounts). Windows — volume serial from
   `GetVolumeInformationW`. Linux — filesystem UUID by matching the mount source device
   against `/dev/disk/by-uuid/*`; `statvfs.f_fsid` only as a corroborating signal.
...
```
            mount detected / startup probe: identity resolved
   ┌─────────┐ ──────────────────────────────────────────▶ ┌─────────┐
   │ offline │                                             │ online  │
   └─────────┘ ◀────────────────────────────────────────── └─────────┘
            unmount event / mount point gone on probe
```
...
start watchers for the volume's roots, schedule a reconciliation scan (§7.3). **offline**:
stop watchers; path rows untouched (availability derivation handles the rest, §8); pending
sidecar writes queue per SIDECARS.md. Detection: DiskArbitration (macOS), `WM_DEVICECHANGE`
(Windows), `/proc/mounts` poll at 5 s (Linux), plus a probe at startup and before any
reconciliation.

### 4.3 Read-only detection

On each online transition: check the mount's read-only flag (`statfs`/`statvfs`,
`FILE_READ_ONLY_VOLUME`), then **verify with a probe** — create-and-delete a temp file in the
watched root (flags lie on network mounts). Store in `volumes.read_only`. Consequence
...
- **Nested roots are forbidden** (decision). Registering inside or above an existing active
  root is rejected; the UI offers "replace existing root with this one" as the only escape
  hatch.
...
`*.photoproof.json` sidecars; known tool/cache dirs, case-insensitive — `@eaDir`,
`__MACOSX`, `*.lrdata`, `*.cocatalogdb`, Capture One session subfolders (`Cache`, `Proxies`,
...
Migrations in `photoproof-core/src/db/`. WAL mode. All timestamps RFC 3339 UTC TEXT. This DDL
is normative; column additions are free, semantic changes require a spec revision.
...
CREATE TABLE volumes (
  ...
  platform_kind   TEXT CHECK (platform_kind IN
                    ('macos-uuid','win-serial','linux-fsuuid','heuristic')),
...
CREATE TABLE images (
  ...
  raw_subtype       TEXT,                       -- 'cr3','nef','arw',…; NULL for non-RAW
  ...
  exposure_time TEXT,                           -- rational as written, e.g. '1/250'
...
CREATE TABLE paths (
  ...
  mtime_ns         INTEGER NOT NULL,            -- ns since epoch, fs-native precision
  state            TEXT NOT NULL CHECK (state IN ('active','stale')),
  stale_reason     TEXT CHECK (stale_reason IN
                     ('moved','deleted','superseded','root-removed')),
...
CREATE TABLE ingest_passes (
  ...
  state         TEXT NOT NULL CHECK
                  (state IN ('pending','running','done','error','skipped')),
  priority      INTEGER NOT NULL DEFAULT 2,     -- §10.3
  attempts      INTEGER NOT NULL DEFAULT 0,
  error         TEXT,                           -- last error; NULL unless state='error'
...
elsewhere within 10 s, flip the reason to `moved`. A move never re-ingests — that is the
...
3. Loaded rows not seen → `stale` (`deleted`); batch move-correlation: a new path whose hash
   matches an image that just lost a path is a move (`stale_reason = 'moved'`).
...
Relink = insert/activate a path row for an existing hash. It never touches `images`, events,
...
Search, the journal panel, retrieval, and export are **fully functional in all three states**:
...
Ingested extensions (case-insensitive): `jpg jpeg png tif tiff webp heic heif dng cr2 cr3 crw
nef nrw arw raf orf ori rw2 pef srw x3f 3fr fff iiq mos mrw kdc dcr sr2 srf erf`. GIF, BMP,
PSD, and video are out of scope for v1 (recorded; PSD and video are the likely first
additions).
...
native-size artifact. Strokes render over `display`; a full-resolution loupe from originals
is post-v1 (the journal needs the proof, not the print).
...
flag. A backfill at backfill priority (§10.3), never an M1 ingest blocker.
...
`OffsetTimeOriginal` → `capture_ts` normalized to UTC with the offset retained in
`capture_tz_offset` (fallback `CreateDate`, else file mtime with `capture_ts` left NULL);
`Make`; `Model`; `LensModel`; `FocalLength`; `ISOSpeedRatings`; `FNumber`; `ExposureTime`;
`Orientation`; pixel dimensions; `GPSLatitude`/`GPSLongitude` (nullable); `ColorSpace` +
embedded ICC presence (consumed by §9.7, not stored beyond what conversion needs).
...
- **EXIF orientation is applied at cache time; cached artifacts are always
  ...
  `images.exif_orientation` and recorded in each stroke event per the kernel, so a tool later
  rewriting orientation metadata is detectable rather than silently rotating marks out from
  under the user. A rotated portrait's preview and a stroke drawn on it must agree on every
  machine and after every rebuild (§13).
- **sRGB at cache time**: embedded ICC profile present → convert to sRGB during artifact
  generation (qcms-class CMS crate); no profile → assume sRGB; EXIF `ColorSpace = AdobeRGB`
  without ICC → convert from built-in AdobeRGB primaries. Wide-gamut display output (P3
  tagging, monitor profiles) is a **known v1 limitation**, recorded here.

### 9.8 Cache layout, eviction, regeneration

- Location: `<app_data>/previews/<h[0..2]>/<h[2..4]>/<hash>-thumb.webp` and `<hash>-disp.webp`
...
- Atomic writes: temp file in the same directory + rename. A crash mid-write leaves the old
  artifact or none, never a torn file; the pass row stays non-`done` and re-runs.
- **Eviction (decision): never, in v1.** 50k images ≈ 50k × (~40 KB + ~700 KB) ≈ 35–40 GB
  worst case — real but acceptable for this audience's hardware. Cache size is computed and
  reported in settings and the debug panel; a manual "clear preview cache" command re-enqueues
  preview passes. LRU eviction is post-v1.
...
Ingest is not a pipeline; it is a set of independent, versioned **passes** over the image
population, each recorded per image in `ingest_passes`. "What happens when models improve" =
a new `pass_version` (or `model_id`) enqueued as a backfill; old rows remain as history.
...
```
   ...
   (absent) ───────────────▶ pending ───────────────────▶ running
                                ▲                            │
                 retry (§10.5)  │              success       │ failure
                                │        ┌──── done ◀────────┤
                                └─── error ◀─────────────────┘
                                       (attempts exhausted: stays error)
   skipped: written directly at enqueue time for inapplicable work
```

On **startup, every `running` row reverts to `pending`** — no leases, no heartbeats; a single
process owns the DB. With idempotent work units (§10.4) this is the entire crash-recovery
story.

### 10.3 Priority and concurrency

Priority (lower = sooner), stored per row: **P0** M1 passes for newly discovered files (live
watcher — the user is probably looking at that folder right now); **P1** M1 passes from
...
a **single DB writer** task receives results over a channel — no worker touches SQLite
directly.
...
done-row + sibling pass enqueues) is one transaction — no window where an image exists
without its queue entries.
...
Core exposes per-pass counters — `(pass_name, pass_version) → {pending, running, done, error,
skipped}` — via query plus a push stream (debounced to 4 Hz) for the dev-build debug panel:
queue depth, files in flight, throughput (files/s; MB/s for hash), ETA, error list. Release
...
(§4.2); availability → `offline` (§8); grid and single-image views serve cached artifacts
with badge data; journal capture against offline images is **fully allowed** (events target
...
Reference machine: 8 physical cores, 32 GB RAM, NVMe ≥ 2 GB/s read, library on the internal
drive. Reference library: 50k images, ~1.5 TB (mixed 40–80 MB RAWs + JPEGs).
...
preview cache eviction; full-resolution loupe decode on demand; wide-gamut display output
(§9.7); GIF/BMP/PSD/video ingestion; libraw FFI fallback (until rawler hits a real-world
format gap).
