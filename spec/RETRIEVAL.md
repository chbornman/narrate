...
All three spaces share the configured embedder (OpenCLIP ViT-H-14/DFN5B-class
preset; kernel) so one text query embedding serves both text spaces and the
CLIP image space.
...
The query is embedded **once** (the OpenCLIP text tower serves S1, the
`image_summary` half of S3, and S4 — same space, same `model_id`).
...
- **Embedder swap:** a new `(vec_kind, model_id)` space = new flat file +
  ...
  after a 7-day grace (instant rollback window). All three vec_kinds cut over
  together (the joint text/image space must be consistent).
