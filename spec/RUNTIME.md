...
2. **At most two long-lived managed service processes**:
   ...
   - Embedder ONNX sessions run in two bounded, role-specific helper processes
     re-entering the installed executable, not as network services. No Python
     anywhere. No Docker anywhere.
3. **Native inference is isolated where recovery requires it.** CLIP and text
   embedders run through killable ONNX Runtime helpers. Silero VAD remains the
   one sanctioned in-process `ort` component. Defense in §3.3.
...
### 3.3 ONNX Runtime isolation — embedder helpers + in-process silero-vad

`Embedder` is implemented through the `ort` crate (ONNX Runtime), loading the
visual and text towers of `ViT-H-14-378-quickgelu__dfn5b` as two ONNX sessions
(the same artifact layout Immich uses). The desktop process does not construct
or run those sessions itself. Each role owns a helper process launched by
re-entering the installed Photoproof executable over a bounded binary stdio
protocol.

Why this is the right native implementation:
...
- The encoder is small, **deterministic, fixed-shape, stateless** — no KV cache, no sampling, no streaming, no prompt surface. A narrow request/response helper protocol is proportionate and keeps model logic in the shared connector.
- ONNX Runtime running these exact models is production-proven at scale in Immich.

The helper boundary is part of the lifecycle contract, not merely a transport
choice. A construction or inference crash fails that role without taking down
the journal. Dispatch, timeout, plan change, retry, and shutdown invalidate the
attempt generation, kill and reap its helper, and prevent stale results from
landing. Model removal and approved GC invalidate an in-flight construction,
kill and reap the helper, drain other consumers, and only then delete files.
Text and CLIP have independent lanes, so one wedged
constructor cannot retain a shared native lock and block the other. Inputs and
frames are bounded before allocation. Provider acceleration is claimed only
from per-node execution evidence; compile-time availability and successful
registration are not execution proof.
...
**Two embedders (resolved cross-spec).** The DFN5B text tower's 77-token CLIP context cannot honor RETRIEVAL's ~512-token annotation chunks — and annotation text is the product's *primary* signal. The runtime therefore hosts **two embedder instances behind the same `Embedder` trait**: the **CLIP embedder** (DFN5B preset above — `image_clip` vectors and short S4 query embeddings only) and the **text embedder** — a small dedicated text-embedding model (B73: EmbeddingGemma-300m q8 default, 768-dim, ~0.33 GB; Qwen3-Embedding-0.6B int8 the configured alternative). Each lives in its own supervised helper process; both remain background-pass-and-query-time components, never part of the capture path. Alternative backend behind the same seam: a GGUF embedding model on llama.cpp's `/v1/embeddings`. RETRIEVAL §3 assigns vec_kinds: `annotation_chunk` + `image_summary` → text embedder; `image_clip` → CLIP embedder.
...
- Download, verification, load/unload dispatch, explicit removal, and the
  destructive half of GC enter one desktop model-operation authority. GC and
  removal first make the model unavailable to new plans, cancel queued
  transfers, kill and reap embedder helpers, and drain supervised consumers;
  only then may they delete files and durably commit the shared installed
  index. A lifecycle converge tick skips rather than blocking behind long
  model I/O and retries on its owned cadence.
...
3. **Embedder recipe**: DFN5B ViT-H-14-378 via `ort` — images/sec on Tier-1 GPU and on CPU; peak VRAM; text-tower parity vs. reference OpenCLIP (cosine ≥ 0.999 on a probe set); stability and helper recovery under a 10k-image batch (§3.3) → gates the GPU-EP default. **Text embedder bake-off**: benchmark **EmbeddingGemma-308M** ([HF blog](https://huggingface.co/blog/embeddinggemma)) alongside Qwen3-Embedding-0.6B as the half-cost candidate — retrieval quality on the RETRIEVAL golden-query set (R6), throughput, and footprint decide the default.
...
- GPU execution-provider stability and actual-provider evidence for the isolated embedder helpers — spike §12.3 gates the default (`device = "auto"` may resolve to CPU in v1 if instability is observed).
