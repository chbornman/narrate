...
BENCH=target/release/pp-voice-bench
echo "=== building bench + both server binaries (release) ==="
cargo build --release -p photoproof-core --bin pp-voice-bench >/dev/null 2>&1
...
run_engine() {
  ...
  t0=$(date +%s.%N)
  local out
  out=$("$BENCH" --wav "$WAV" --model-dir "$model_dir" --server "$server" \
        --expect "$EXPECT" --json 2>/dev/null || echo '{}')
  t1=$(date +%s.%N)
  ...
  # WER (gated) out of the bench JSON, if present. serde emits compact JSON,
  # so the gated score is `..."gated":{"wer":<x>,...`.
  local wer; wer=$(printf '%s' "$out" | sed -n 's/.*"gated":{"wer":\([0-9.]*\).*/\1/p' | head -1)
  [[ -z "$wer" ]] && wer="n/a"

  echo "## $name"
  echo "  wall:        ${wall}s   (audio ${AUDIO_S}s)"
